<div>
                ...
                <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
                    ...
                    <tbody>
                        @forelse($productItems as $productItem)
                            <tr>
                                <td class="pl-0">
                                    {{ $productItem->inventoryItem->name }}
                                </td>
                                <td class="pr-0 text-left">
                                    <x-edit-record-button href="javascript:void(0)" wire:click="selectItem({{ $productItem->inventory_item_id }}, 'update')" />
                                    <x-delete-record-button href="javascript:void(0)" wire:click="selectItem({{ $productItem->inventory_item_id }}, 'delete')"  />
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    @include('livewire.dashboard.product-items.create')
    @include('livewire.dashboard.product-items.update')
</div>
